...
from .data_manager import core_data_path, cog_data_path
from .utils.json_driver import JSON

log = logging.getLogger("tbv.config")
...
class Config:
    ...
    @classmethod
    def get_cog_conf(cls, cog_instance, force_registration: bool = False):
        ...
        if cog_instance is not None:
            cog_path = cog_data_path(cog_instance)

        cog_name = cog_path.stem

        driver = JSON(cog_name, data_path=cog_path)
        conf = cls(
            cog_name=cog_name,
            driver=driver,
    ...
    def get_core_conf(cls, force_registration: bool = False):
        ...
        driver = JSON("Core", data_path=core_path)
        conf = cls(
            cog_name="Core",
            driver=driver,
            force_registration=force_registration
    ...
    async def _all_from_scope(self, scope: str):
        """Get a dict of all values from a lar scope of data.
